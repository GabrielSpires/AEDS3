...
int somaAchada = 0; //Variável usada pra interromper os testes
...
void fooR(int* src, int* v, int srcIndex, int vIndex, int srcSize, int vSize, int soma){
	...
	if(vIndex==vSize){
		...
		for(i=srcIndex; i<srcSize; i++){
			v[vIndex] = src[i];
			fooR(src, v, i+1, vIndex+1, srcSize, vSize, soma);
		}
	}
}

void foo(int* src, int size, int k, int soma){
	int *v;
	v = (int*)calloc(k, sizeof(int));
	fooR(src, v, 0, 0, size, k, soma);
}
